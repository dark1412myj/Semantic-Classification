...
def get_eosc_data(): # evaluation sentiment classification
        ...
        for j, word in enumerate(line):
            if word == 'B':
                ...
                while j + 1 < len(line) and sentences_test_Y[i][j + i_count + 1] == 'I':
                    i_count += 1
                temp_v = np.zeros(WORD_DIM).tolist()
    ...
    for i,line in enumerate(train_Y):
        line[ int(list_train[i]) ] = 1
    for i,line in enumerate(test_Y):
        line[ int(list_test[i]) ] = 1
...
def test():
    ...
    sentences_train_Y = []
    sentences_test_Y = []
    k = get_basic_data(sentences_test_X,sentences_test_Y,'./data/test')
    get_basic_data(sentences_train_X,sentences_train_Y,'./data/train')
    print(k)
